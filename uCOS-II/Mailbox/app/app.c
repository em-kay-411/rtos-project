...
*****************************************************************************************
*
* Program Name : Mailbox implementation in uCos-II for message passing
...
*				 Task0 accepts a message from the PC keyboard through UART and
*				 sends the message to TASK1 through Mailbox. Concurrently, if TASK1
*				 receives any message through the Mailbox, it displays that message 
*				 on LCD.
*
* 
...
*                                    TASK-0 : AppTask0()
*
* Description : This task accepts a message from the user upto 16 characters. When ENTER
...
static  void  AppTask0 (void *p_arg)
{
    ...
    {     
		...
		printf("TASK0: Enter a message from PC Keyboard (max 16 chars): ");
		for(i=0;i<16;i++)				/* Accept message from the user	*/
		{
			temp = UART_GetChar();
			if(temp == 0x0D)
				break;					/* if ENTER is pressed i.e. <CR>, then break out of the for loop */
			UART_PutChar(temp);			/* Echo the key pressed */
			msg[i] = temp;				/* Store the key pressed in the msg array */
		}
		msg[i] = '\0';					/* Insert NULL character at the end of string */
		while(UART_GetChar() != 0x0A);	/* wait till <LF> new line character is recieved after ENTER */

		OSMboxPost(TxMbox, (void*)msg);		 /* Post the message to the Mailbox */
		printf("\n\rTASK0: Message Sent....\n\r");
...
*                                    TASK-1 : AppTask1()
*
* Description : This task waits for a message to arrive in the mailbox. If any message is
